...
type
  ...
  public
    ...
    function  mightHaveRegisterEx     : Boolean;    
    function  mightHavePointer        : Boolean; 
    ...
    function  ramOPX : smallInt;                  { ram[regs[IP] + 3].opHex   } 
    ...
    function  ramTo3 : string;                    { ram[regs[IP] + 3].opToken } 
...
begin
  if n in [5,7] then result := true else result := false
end;
...
begin
  if FormMain.CheckBoxAsmLog.Checked then
  begin
    FormMain.MemoAsmLog.Lines.Add('Might have Pointer');
  end;

  if currTokStr = 'BP' then
  begin
    currToken.opHex   := 7;
    currToken.opAscii := '07';
    currToken.opBin   := '00000111';
    result := true;
    incIII
  end
  else if currTokStr = 'SP' then
  begin
    currToken.opHex   := 5;
    currToken.opAscii := '05';
    currToken.opBin   := '00000101';
...
function Taaa.mightHaveRegisterEx : Boolean;
begin
  if FormMain.CheckBoxAsmLog.Checked then
  begin
    FormMain.MemoAsmLog.Lines.Add('Might have register');
...
begin
  aCmd := prevToken;
  arg1 := currToken;
  if (mightHaveRegister or mightHavePointer) then                { INC    AL      }
  begin
  ...
  begin
    error('Expected AL, BL, CL, DL or BP.  Got ' + currTokStr, currToken)
  end
end;
...
begin
  mov := prevToken;
  arg1 := currToken;
  if (arg1.Token<>'SP') AND mightHaveRegister  then { MOV    AL      }
  begin
             ...
             listAdd(TABChar + mov.Token + TABChar +
                ...
                begin
                  ...
                  begin
					...
					begin
					  error('Expected '']'' Got ' + currTokStr, currToken)
					end
                  end    
                  else if mightHaveRegisterEx then                  { MOV    AL,[AL  }
                       begin
                         if mightHave(']') then                     { MOV    AL,[AL] }
                         begin
                              ...
                              begin
                                ...
                                begin
                                  ...
                                  begin
                                    ...
                                    generate(arg1);
                                    arg2.token := '[' + arg2.token;
                                    argx.token := '+' + argx.token + ']';
                                    generate(arg2);
                                    generate(argx);
                                    mov.selLength := 1 + (argx.selStart + argx.selLength) - mov.selStart;
                                    listAdd(TABChar + mov.Token + TABChar +
                                            arg1.token + ',' + arg2.token + argx.token +
                                            TABChar + '; [' +
                                            IntToHex(ramAddr - 3, 2) + ']  ' +
                                            mov.opAscii + ' ' +
                                            arg1.opAscii + ' ' +
                                            arg2.opAscii + ' ' + argx.opAscii, false)
                                  end
                                end
                                else
                                begin
         ...
         begin
           ...
           begin
            ...
            begin
              ...
              else if mightHaveRegisterEx then                      { MOV    [AL     }
                   begin
                     if mightHave(']') then                         { MOV    [AL]    }
                     begin
                       if mustHave(',') then                        { MOV    [AL],   }
                       begin
                         arg2 := currToken;
                              ...
                              begin
                                ...
                                begin
                                  ...
                                  begin
                                    ...
                                    mov.selLength := (arg2.selStart + arg2.selLength) - mov.selStart;
                                    listAdd(TABChar + mov.Token + TABChar +
                                            arg1.token + argx.token + ',' + arg2.token +
                                            TABChar + '; [' +
                                            IntToHex(ramAddr - 3, 2) + ']  ' +
                                            mov.opAscii + ' ' +
                                            arg1.opAscii + ' ' +
...
{ Decrement SP }          if (HexFF AND regs[SP] > 0) then
                        ...
                        begin
                          ...
                          if (validReg(ramOP1) or validpoi(ramOP1)) then
                          begin
...
begin
  ...
  begin
    if validPoi(reg1) then
    begin
      if validPoi(reg2) then
      begin
        logRegs;
        log('FETCH');
        log(#9 + 'Fetch ' + ramTo0);
        ...
        regAssign(reg1, regs[reg2], dontSetFlags);
        regAssign(IP, regs[IP] + 3, dontSetFlags);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        log(#9 + 'Fetch ' + ramTo0);
        log(#9 + 'Fetch ' + ramTo1);
        log(#9 + 'Fetch ' + ramTo2 + ramTo3);
        log('');
        log('EXECUTE');
        log(#9 + ramTo0 + #9 + ramTo1 + ',' + ramTo2 + ramTo3);
        log('');

        regAssign(reg1, ram[HexFF AND (regs[reg2] + regx)].opHex, dontSetFlags);
        regAssign(IP, regs[IP] + 4, dontSetFlags);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        log('FETCH');
        log(#9 + 'Fetch ' + ramTo0);
        log(#9 + 'Fetch ' + ramTo1 + ramTo3);
        log(#9 + 'Fetch ' + ramTo2);
        log('');
        log('EXECUTE');
        log(#9 + ramTo0 + #9 + ramTo1 + ramTo3 + ',' + ramTo2);
        log('');
        ramAssign((regs[reg1] + regx), regs[reg2]);
        regAssign(IP, regs[IP] + 4, dontSetFlags);
        logIP('4');
      end
      else
...
procedure Taaa.imovRR;           { MOV    BL,AL    D5 01 00 }
var reg1, reg2 : smallInt;
begin
  reg1 := ramOP1;
  reg2 := ramOP2;
